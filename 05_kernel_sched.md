...
- 交互任务响应快（需要调度周期短，以保证线程能及时的获得CPU时间）
- 批处理任务吞吐量大（需要减小调度带来的overhead，即调度器占用的CPU时间）
- SMP高效多核，高cache和TLB命中率（减小线程切换甚至是migration对时间和空间局部性的破坏）



# task scheduler

调度器以runqueue为核心，然后由`sched_entity`来记录调度对象的动态信息。调度实体以红黑树为基本数据结构，存储在runqueue里。
...
#### priority vs sched_class

`sched_class` defines methods for different schedulers, then determines how a group of tasks (in the class) are scheduled. `policy` and `prio` decides which class a task belongs to:

![image-20250804222333210](./.05_kernel_sched/image-20250804222333210.png)
...
![image-20250804195541010](./.05_kernel_sched/image-20250804195541010.png)
...
![image-20250622165658422](./.05_kernel_sched/image-20250622165658422.png)
...
![image-20250622165833099](./.05_kernel_sched/image-20250622165833099.png)
...
Implementation of scheduler, which will be called before swapping (/preempting) to choose the next task to swap in.
...
Increment of the `se->vruntime`, used to get the eclipsed time since last update of `cfs_rq->avg_vruntime`.

![image-20250602223724313](./.05_kernel_sched/image-20250602223724313.png)
...
带缓存的红黑树只是一个常规的`rb_root`，加上一个额外的指针来缓存最左边的节点。这使得`rb_root_cached`可以存在于rb_root存在的任何地方，并且只需增加几个接口来支持带缓存的树
...
Schedule latency is the duration in which all runnable processes are expected to be scheduled at least once. It defines the "scheduling period" during which the CPU time is divided fairly among all runnable tasks.
...
The schedule latency defines the time frame in which all tasks should be scheduled, and the time-slice per task is derived from dividing this latency by the number of tasks, respecting the **minimum granularity**.
...
This issue is handled in `place_entity()`, when enqueueing a task.

For CFS:

- New Tasks: set `vruntime` as `cfs_rq->min_vruntime`, but with some penalty (presuming that it has already consumed its ration in current period);
- Waking Tasks: update its `vruntime` as `cfs_rq->min_vruntime` (this is a penalty), but then with some awards as it just got the resources it needs. And make sure the new value won't be smaller than the previous `se.vruntime`. (Preemption only be triggered by system ticks, which may come much later) 



For EEVDF:

- New Tasks: vlag = 0, and vruntime = avg_vruntime;
- Waking Tasks: vlag = preserved_lag, and vruntime = avg_vruntime - vlag;
...
- Interactive Workload responsiveness (CFS prioritizes long-term fairness over short-term latency guarantees, a design choice that manifests in degraded **interactive performance** under heavy CPU contention.
- CFS's strict adherence to vruntime ordering forces interactive tasks to wait their turn even when user-perceived latency becomes critical.
- Newly created tasks and Waking tasks are punished by the fact that every task will consume its time-slice before being preempted. So that the former will have a latency longer than the sched_period, and the later will suffer from big time-slice of the running tasks.
- Context switching overhead is proportional to time-slice size, which is in turn determines the sched_period. So, higher the throughput, larger the sched_latency, vise versa. 



**What is lacking is a way to ensure that some processes can get access to a CPU quickly without necessarily giving those processes the ability to obtain more than their fair share of CPU time.** (https://lwn.net/Articles/925371/)



##### Data Structure Constraints

CFS's **red-black** tree enables O(log n) insertion and deletion operations. (Heap-based schedulers achieve 12-15% higher throughput in workloads exceeding 10,000 runnable threads).

The tree's **cache-unfriendly** memory access patterns exacerbate performance issues on NUMA architectures.



#### EEVDF scheduler

Earliest Eligible **Virtual Deadline** First, A newer scheduler variant alongside CFS to **address latency requirement**.

The `commit` adding this implementation is `git show 147f3efaa241`

- EEVDF scheduler adds a `new dimension`, **request size** of timeslice, to each schedule entity, in addition to **weight** of CPU time. The **request size** and **weight** together determine the **schedule period** of a process. (`sched_period = (slice / weight) * total_weight`)

- The name **EEVDF**/EDF actually describes how to pick the next task, based on the two dimensions above. EEVDF picks task with `lag` greater or equal to zero (**eligible**) and calculates a virtual deadline (**VD**) for each, selecting the task with the **earliest VD** to execute next. Normally, the `lag` value becomes positive after each `sched_period`.

- **Guess**: `SCHED_BATCH` policy should be set with bigger time-slice size and higher nice value, then it will wait all `SCHED_NORAML` policy threads finish their work before running on CPU with big time-slice to fulfill a big throughput by leveraging the cache locality.

This transition marks a fundamental shift from CFS's weighted fair queuing (**WFQ**) model to a deadline-oriented scheduling paradigm that better accommodates **latency-sensitive** workloads while maintaining (**weight** based) fairness across processes. It aims to improve fairness, **responsiveness**, and scalability while addressing **limitations** in CFS.

- **Tick driven** preemption is driven by `request/slice` completion; 

- while **wakeup** preemption is driven by the `deadline`. (reduces wakeup-to-execution latency)
...
- `deadline = vruntime + (slice / weight)`   (**earliest time** should have received its due CPU time)

- `deadline = vruntime + (slice / weight) * total_weight`  ? (latest time)

Deadline is the time when a process should have received its due CPU timeslice, while all other processes get their partitions too. (This is why it multiplies the **total_weight** after being normalized)
...
`runtime`s are supposed to be **evenly** distributed to all entities in `runqueue`, so the `rq.avg_vruntime` means the **normalized** virtual runtime should be entitled to each task:
...
**sched_latency_nice**: New parameter (-20 to 19) influencing time slice allocations

The latency_nice value maps to time slice durations using an **exponential scale**:

- Lower values -> Smaller slices -> even Earlier deadlines
- Higher values -> Larger slices -> even Later deadlines



##### Occasion to reschedule

Always preempt a thread after its timeslice consumed? (When no New/Waking tasks)
...
![image-20250602224504541](./.05_kernel_sched/image-20250602224504541.png)

![image-20250602224638082](./.05_kernel_sched/image-20250602224638082.png)
...
A heap is a specialized tree-based data structure that satisfies the **heap property** and is typically implemented as a complete binary tree. This means the tree is completely filled on all levels except possibly the last, which is filled from left to right. The heap property ensures a specific ordering between parent nodes and their children:

- In a **max heap**, for any given node *C*, if *P* is the parent of *C*, then the key (value) of *P* is greater than or equal to the key of *C*. Consequently, the root node holds the maximum value in the heap.
- In a **min heap**, the key of the parent node *P* is less than or equal to the key of the child node *C*, so the root node holds the minimum value.

This property guarantees that **the highest (or lowest) priority element is always at the root**, make heaps very useful for implementing priority queues efficiently.



Heaps are commonly implemented using **arrays** rather than explicit tree structures. The relationship between parent and child nodes is defined by indices in the array:

For a node at index *i*: (This can be deduced my mathematics)

- The left child is at index *2i + 1*
- The right child is at index *2i +2*
- The parent is at index *[(i - 1) / 2]*

This implicit structure allows efficient navigation and manipulation without extra memory overhead for pointers.



Operations on a Heap:

- **Insertion**: Add the new element at the end of the array (bottom of the tree) and then "sift up" or "heapify up" to restore the heap property by swapping it with its parent until the property is satisfied.
- **Deletion** (usually of the root): Replace the root with the last element in the array, remove the last element, and then "sift down" or "heapify down" to restore the heap property by swapping with the larger (max heap) or smaller (min heap) child.
- **Heapify**: A process to **build a heap** from an unordered array by repeatedly applying sift-down operations starting from the lowest non-leaf nodes.
...
https://s3.shizhz.me/linux-sched/cfs-sched/group
...
![image-20250622165618077](./.05_kernel_sched/image-20250622165618077.png)
...
![image-20250702192644551](./.05_kernel_sched/image-20250702192644551.png)



### update_rq_clock_task

![image-20250702192901225](./.05_kernel_sched/image-20250702192901225.png)

![image-20250702192919024](./.05_kernel_sched/image-20250702192919024.png)



### update_rq_clock_pelt

![image-20250702193314186](./.05_kernel_sched/image-20250702193314186.png)



## resched_curr

![image-20250702202627153](./.05_kernel_sched/image-20250702202627153.png)
...
The state of a task will be set to `TASK_INTERRUPT` if it's interrupted and the interrupt process is not finished to let scheduler have change to switch it out or recover it's execution. In this state, it cannot be scheduled on current or other CPUs.
